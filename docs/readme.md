...
- **Schema 隔离**：插件复用宿主数据库实例，但使用**独立 schema**（例如 `note`），避免污染 `public`
- **对接 PowerX**：
...
| 变量名                   | 说明                                           | 示例                                                        |
| ------------------------ | ---------------------------------------------- | ----------------------------------------------------------- |
| `PX_BIND_ADDR`           | 插件监听地址                                   | `:8091`                                                     |
| `PX_DB_DSN`              | Postgres 连接串                                | `postgres://user:pwd@127.0.0.1:5432/powerx?sslmode=disable` |
| `PX_DB_SCHEMA`           | 插件独立 schema                                | `note`                                                      |
| `PX_LOG_LEVEL`           | 日志级别                                       | `info` / `debug`                                            |
| `PX_RUN_MIGRATE`         | 仅执行迁移后退出                               | `true/false`                                                |
| `PX_DEV_MODE`            | 开启本机调试旁路（仅开发）                     | `1`                                                         |
| `PLUGIN_CTX_HMAC_SECRET` | \*\*（HMAC 模式）\*\*平台 → 插件上下文签名密钥 | base64 编码的 32B                                           |
| `PLUGIN_CTX_KID`         | \*\*（HMAC）\*\*密钥标识                       | `com.xxx:v1`                                                |
| `PX_CTX_JWKS_URL`        | \*\*（JWT 模式）\*\*平台公钥 JWKS              | `http://powerx/_p/_internal/jwks`                           |
| `PX_CTX_ISSUER`          | 上下文签发者                                   | `powerx-auth`                                               |
| `PX_CTX_AUDIENCE`        | 受众                                           | `powerx-plugin`                                             |
| `PX_CTX_TTL`             | 上下文有效期                                   | `300s`                                                      |
...
PX_RUN_MIGRATE=true go run ./backend/cmd/plugin/main.go
...
export PX_DEV_MODE=1   # 开启 dev bypass，仅开发期
...
```bash
curl "http://localhost:8080/_p/com.powerx.plugins.note/api/v1/ping"
curl "http://localhost:8080/_p/com.powerx.plugins.note/api/v1/notes" -H "X-PowerX-CTX: ..." -H "X-PowerX-CTX-JWT: ..."
```
...
ALTER TABLE note.note ENABLE ROW LEVEL SECURITY;

-- 依赖每请求事务里的：SET LOCAL app.tenant_id = <tenant>
CREATE POLICY p_tenant_isolation ON note.note
...
> 开发期可用 `PX_DEV_MODE=1` 开启受控旁路（仅本机，**上线务必关闭**）。
...
agents:
  - id: "note.assistant"
    ...
    default_tools: ["note.note.create", "note.note.query"]
...
- **模式 B**：插件单跑直连 `:8091/v1/...`，开发期用 `PX_DEV_MODE=1` 旁路注入 `tenant_id`
...
run:        # 启动（PX_BIND_ADDR=:8091 PX_DB_SCHEMA=note）
migrate:    # 运行迁移
package:    # 打包 plugin.zip（plugin.yaml + backend/bin/... + web-admin）
docker:     # 构建镜像 powerx-plugin-note:<version>
```
...
```bash
docker build -t powerx-plugin-note:0.1.0 .
docker run --rm -p 8091:8091 \
  -e PX_BIND_ADDR=":8091" \
  -e PX_DB_DSN="postgres://user:pwd@host:5432/powerx?sslmode=disable" \
  -e PX_DB_SCHEMA="note" \
  -e PX_CTX_JWKS_URL="http://powerx/_p/_internal/jwks" \
  -e PX_CTX_ISSUER="powerx-auth" \
  -e PX_CTX_AUDIENCE="powerx-plugin" \
  powerx-plugin-note:0.1.0
```
