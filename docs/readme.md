...
* **Schema 隔离**：插件复用宿主数据库实例，但使用**独立 schema**（例如 `scrum`），避免污染 `public`
* **对接 PowerX**：
...
| `PX_DB_SCHEMA`           | 插件独立 schema                   | `scrum`                                                     |
...
```bash
curl "http://localhost:8080/_p/com.powerx.plugins.scrum/api/v1/ping"
curl "http://localhost:8080/_p/com.powerx.plugins.scrum/api/v1/tasks" -H "X-PowerX-CTX: ..." -H "X-PowerX-CTX-JWT: ..."
```
...
ALTER TABLE scrum.task ENABLE ROW LEVEL SECURITY;

-- 依赖每请求事务里的：SET LOCAL app.tenant_id = <tenant>
CREATE POLICY p_tenant_isolation ON scrum.task
...
agents:
  - id: "scrum.assistant"
    ...
    default_tools: ["scrum.task.create","scrum.task.query"]
...
run:        # 启动（PX_BIND_ADDR=:8091 PX_DB_SCHEMA=scrum）
migrate:    # 运行迁移
package:    # 打包 plugin.zip（plugin.yaml + backend/bin/... + web-admin）
docker:     # 构建镜像 powerx-plugin-scrum:<version>
```
...
```bash
docker build -t powerx-plugin-scrum:0.1.0 .
