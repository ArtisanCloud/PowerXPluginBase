...
func NewCredentialService(cfg *config.Config, repo *repo.CredentialsRepo) *CredentialService {
...
func (s *CredentialService) SavePlainCredentials(ctx context.Context, tenantID int64, pluginID, clientID, clientSecret string) error {
    if tenantID <= 0 { return errors.New("invalid tenant_id") }
    if pluginID == "" || clientID == "" || clientSecret == "" { return errors.New("missing required fields") }
