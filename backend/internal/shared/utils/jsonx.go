...
func ToString(v any) string {
    ...
    case map[string]any:
        if lv, ok := t["low"].(float64); ok { return strconv.FormatInt(int64(lv), 10) }
...
func ToInt64(v any) int64 {
    ...
    case string:
        if t == "" { return 0 }
        if iv, err := strconv.ParseInt(t, 10, 64); err == nil { return iv }
        return 0
    case map[string]any:
        if lv, ok := t["low"].(float64); ok { return int64(lv) }
        return 0
    default:
        return 0
    }
...
func FlattenMemberMap(src map[string]any) map[string]any {
    ...
    if arrv := FirstNonNil(src, "team_ids", "teamIds"); arrv != nil {
        if arr, ok := arrv.([]any); ok {
            var ids []int64
            for _, v := range arr { ids = append(ids, ToInt64(v)) }
            out["team_ids"] = ids
        }
    }
    return out
